...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    ...
    if (!_dataDownJackets.count) {
        ...
        // spacing and size adjust
        CGFloat subSpacing = 7, subWidth = self.height - 20;
        CGFloat scrollContentWidth = (subWidth + subSpacing) * _dataDownJackets.count + _leftPadding - subSpacing;
        ...
        [_items enumerateObjectsUsingBlock:^(DownJacketItem * _Nonnull button, NSUInteger idx, BOOL * _Nonnull stop) {
            button.size = CGSizeMake(subWidth, subWidth);
            button.x = (subWidth + subSpacing) * idx + _self.leftPadding;
