...
public extension Bundle {
    ...
    public func image(named: String) -> UIImage? {
        ...
        if #available(iOS 13.0, *) {
            image = UIImage(named: named, in: self, with: nil)
        } else {
            image = UIImage(named: named, in: self, compatibleWith: nil)
        }
        return image
    }
    
    /// 读取Bundle内文件
    public func contentsOfFile(named: String?, ofType type: String? = nil) -> Any? {
